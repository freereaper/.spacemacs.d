...
(defvar find-and-ctags-prune-patterns
  '(;; VCS
    "*/.git/*"
    "*/.svn/*"
    "*/.cvs/*"
    ...
    ;; Python
    "*.pyc")
  "List of directory/file patterns to not descend into when listing files in `find-and-ctags'.")
  ...
  (let (rlt)
    (if find-and-ctags-windows-p
        (cond
         ...
         ;; do not use the ctags which provided by emacs
         (t (setq rlt "/usr/bin/ctags"))))
    (if rlt rlt name)))
...
(defun ctags--ffip-prune-patterns ()
  (mapconcat (lambda (pat) (format "-iwholename \"%s\"" pat))
             find-and-ctags-prune-patterns " -or "))
    ...
    ;; use relative directory because TAGS is shared between Cygwin and Window
      (dolist (row opts-matrix)
        (let* ((default-directory dir)
               (find-opts (car row))
               (ctags-opts (cadr row))
               (cmd (format "%s -L . \\( %s \\) -prune -o -type f %s | %s -e %s -L -"
                            find-exe
                            (ctags--ffip-prune-patterns)
